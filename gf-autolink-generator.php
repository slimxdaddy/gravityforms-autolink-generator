<?php
/**
 * Plugin Name: V8 GF Autolink Generator (/f/{slug}) + Forms List “Link” Column
 * Description: Clean URLs like /f/{slug} rendered with your theme (no preview). Adds a “Link” column on the Forms list where you can set the slug. Uses a proper POST (admin-post.php) so saving works even with a page refresh. Also hides meta + shrinks title on virtual /f/ pages.
 * Version: 1.4.0
 * Author: Web V8
...
final class V8_GF_Form_Permalinks {
	const BASE       = 'f';                     // URL base => /f/{slug}
	const QUERY_VAR  = 'v8_f';
	const OPTION_KEY = 'v8_gf_permalinks_map'; // [ form_id => slug ]
	...
	public static function init() : void {
		...
		// Admin: Forms list column + POST handler
		...
		// Styling for virtual form pages
	...
	public static function handle_save() {
		...
		if ( ! class_exists( 'GFAPI' ) ) wp_die( 'Gravity Forms not active' );

		$form = GFAPI::get_form( $form_id );
		if ( is_wp_error( $form ) || ! $form ) wp_die( 'Form not found' );
		...
		// Redirect back to the forms list so the new link is visible immediately.
		$dest = admin_url( 'admin.php?page=gf_edit_forms' );
		wp_safe_redirect( $dest );
		exit;
	}

	/* ---------- Styling for virtual /f/ pages ---------- */
	...
	public static function head_styles() : void {
		...
		<style id="v8-form-page-css">
			...
			body.v8-form-page .article-meta { display:none !important; }
			body.v8-form-page .entry-title,
			body.v8-form-page .page-title,
			body.v8-form-page h1 { font-size:2rem !important; line-height:1.2 !important; margin-bottom:1rem !important; }
			body.v8-form-page .entry-header { margin-bottom: 1rem !important; }
		</style>
	<?php }
}
